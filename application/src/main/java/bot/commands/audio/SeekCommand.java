...
public class SeekCommand extends Command {
	public SeekCommand() {
		...
		this.options = List.of(Option.createOption(Response.SEEK_POSITION, true, 0));
	}
	...
	protected void execute(CommandEvent event) {
		...
		int seekTime = getSeekTime(event.getOption(Response.SEEK_POSITION).getAsString());
		...
		event.reply(String.format(TextChannelResponses.SEEKING_TO_INFORMATION, TimeUtils.timeString(seekTime)));
