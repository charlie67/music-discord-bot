 ...
 * menu Extension. <br>
 * Using several fields, a menu can define properties that make it unique and complex while
 * maintaining a low level of development. <br>
 * All classes extending this class can define any number of these fields in a object constructor
 * and then create the menu action/response in the abstract {@link
 * UserContextMenu#execute(UserContextMenuEvent)} body:
 *
 ...
 * }</code></pre>
 *
 * <p>Execution is with the provision of a UserContextInteractionEvent-CommandClient wrapper called
 * a {@link UserContextMenuEvent} and is performed in two steps:
 *
 * <ul>
 *   <li>{@link UserContextMenu#run(UserContextMenuEvent) run} - The menu runs through a series of
 *       conditionals, automatically terminating the command instance if one is not met, and
 *       possibly providing an error response.
 *   <li>{@link UserContextMenu#execute(UserContextMenuEvent) execute} - The menu, now being cleared
 *       to run, executes and performs whatever lies in the abstract body method.
 * </ul>
 *
 * @author Olivia (Chew)
 */
public abstract class UserContextMenu extends ContextMenu {
  /**
   * Runs checks for the {@link UserContextMenu} with the given {@link MessageContextMenuEvent} that
   * called it. <br>
   * Will terminate, and possibly respond with a failure message, if any checks fail.
   *
   * @param event The UserContextMenuEvent that triggered this Context Menu
   */
      ...
      for (Permission p : userPermissions) {
        ...
        if (p.isChannel()) {
          if (!event.getMember().hasPermission(event.getGuildChannel(), p)) {
            terminate(
                event,
                String.format("%s%s%s", event.getClient().getError(), p.getName(), "channel"));
            return;
          }
        } else {
          if (!event.getMember().hasPermission(p)) {
            terminate(
                event,
                String.format("%s%s%s", event.getClient().getError(), p.getName(), "server"));
            return;
          }
        }
      }
          ...
          if (p.name().startsWith("VOICE")) {
            ...
            if (vc == null) {
              ...
              terminate(
                  event,
                  String.format(
                      "%s%s%s", event.getClient().getError(), p.getName(), "voice channel"));
              return;
            }
          } else {
  ...
  /**
   * The main body method of a {@link UserContextMenu}. <br>
   * This is the "response" for a successful {@link UserContextMenu#run(UserContextMenuEvent)}
   *
   * @param event The {@link UserContextMenuEvent} that triggered this menu.
   */
  ...
  public CommandData buildCommandData() {
    ...
    if (this.userPermissions == null) data.setDefaultPermissions(DefaultMemberPermissions.DISABLED);
    else data.setDefaultPermissions(DefaultMemberPermissions.enabledFor(this.userPermissions));
