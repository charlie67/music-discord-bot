...
/**
 * A wrapper class for a {@link MessageReceivedEvent MessageReceivedEvent}, {@link CommandClient
 * CommandClient}, and String user arguments compatible with all {@link bot.utils.command.Command
 * Command}s.
 *
 ...
 * response to the {@link MessageChannel MessageChannel} the MessageReceivedEvent was in. <br>
...
public class BotCommandEvent implements CommandEvent {
   ...
   * It is a generated wrapper for a {@link MessageReceivedEvent MessageReceivedEvent}.
   *
   ...
   * anyone. <br>
   * Useful for splitting long messages so that they can be sent in more than one {@link Message
   * Message} at maximum potential length.
   *
  ...
  /**
   * Returns the underlying {@link MessageReceivedEvent MessageReceivedEvent} for this CommandEvent.
   *
  ...
  /**
   * Returns the {@link CommandClient CommandClient} that initiated this CommandEvent.
   *
  ...
  /**
   * Links a {@link Message Message} with the calling Message contained by this CommandEvent.
   *
   * <p>This method is exposed for those who wish to use linked deletion but may require usage of
   * {@link MessageChannel#sendMessage(CharSequence)} (Message) MessageChannel#sendMessage()} or for
   * other reasons cannot use the standard {@code reply()} methods.
   *
   * <p>If the Message provided is <b>not</b> from the bot (IE: {@link SelfUser SelfUser}), an
   ...
   * response as a {@link Message Message} automatically does {@link
  ...
  /**
   * Replies with a {@link MessageEmbed MessageEmbed} and then queues a {@link Consumer}.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()} with the provided Consumer
  ...
  /**
   * Replies with a {@link Message Message} and then queues a {@link Consumer}.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   ...
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()}.
   *
   * <p>This method uses {@link MessageChannel#sendFiles(FileUpload...)} (File, String,
  ...
  public void reply(String message, File file, String filename) {
    event.getChannel().sendFiles(FileUpload.fromData(file)).addContent(message).queue();
  }
  ...
  /**
   * Replies with a {@link MessageEmbed MessageEmbed} if possible, or just a String message if it
   * cannot send the embed.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()}.
  ...
  /**
   * Replies with a String message and a {@link File} with the provided name, or a default name if
   * left null.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   ...
   * CommandEvent#reply(File, String)} and {@link CommandEvent#reply(String, File, String)
   ...
   * comes directly from a thrown {@link Exception}, and thus a thrown IOException is grounds for
   * the sending of the alternate message.
   *
   * @param message A String message to reply with
   * @param file The File to reply with
   * @param filename The filename that Discord should display (null for default).
  ...
  /**
   * Replies with a String message sent to the calling {@link User User}'s {@link PrivateChannel
   * PrivateChannel}.
   *
  ...
  /**
   * Replies with a {@link MessageEmbed MessageEmbed} sent to the calling {@link User User}'s {@link
  ...
  /**
   * Replies with a {@link Message Message} sent to the calling {@link User User}'s {@link
   * PrivateChannel PrivateChannel}.
   *
   * <p>If the User to be Direct Messaged does not already have a PrivateChannel open to send
   * messages to, this method will automatically open one.
  ...
  public void replyInDm(Message message) {
    if (event.isFromType(ChannelType.PRIVATE)) reply(message);
      ...
      event
          .getAuthor()
          .openPrivateChannel()
          .queue(pc -> pc.sendMessage(MessageCreateData.fromMessage(message)).queue());
    }
  }
  ...
  public void replyInDm(Message message, Consumer<Message> success) {
    if (event.isFromType(ChannelType.PRIVATE))
      getPrivateChannel().sendMessage(MessageCreateData.fromMessage(message)).queue(success);
    else {
      event
          .getAuthor()
          .openPrivateChannel()
          .queue(pc -> pc.sendMessage(MessageCreateData.fromMessage(message)).queue(success));
    }
  }
  ...
  /**
   * Replies with a String message and a {@link File} with the provided name, or a default name if
   * left null, and sent to the calling {@link User User}'s {@link PrivateChannel PrivateChannel}.
   *
   * <p>If the User to be Direct Messaged does not already have a PrivateChannel open to send
   * messages to, this method will automatically open one.
   *
   ...
   * MessageChannel#sendFile(File, String, AttachmentOption...)} to send the File. For more
  ...
  public void replyInDm(String message, File file, String filename) {
    if (event.isFromType(ChannelType.PRIVATE)) reply(message, file, filename);
      ...
      event
          .getAuthor()
          .openPrivateChannel()
          .queue(pc -> pc.sendFiles(FileUpload.fromData(file)).addContent(message).queue());
    }
  }
  ...
  /**
   * Replies with a String message and a prefixed success emoji and then queues a {@link Consumer}.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()} with the provided Consumer
  ...
  /**
   * Replies with a String message and a prefixed warning emoji and then queues a {@link Consumer}.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()} with the provided Consumer
  ...
  /**
   * Replies with a String message and a prefixed error emoji and then queues a {@link Consumer}.
   *
   * <p>The {@link net.dv8tion.jda.api.requests.RestAction RestAction} returned by sending the
   * response as a {@link Message Message} automatically does {@link
   * net.dv8tion.jda.api.requests.RestAction#queue() RestAction#queue()} with the provided Consumer
  ...
  /**
   * Uses the {@link CommandClient#getScheduleExecutor() client's executor} to run the provided
   * {@link Runnable Runnable} asynchronously without blocking the thread this is called in.
   *
   * <p>The ScheduledExecutorService this runs on can be configured using {@link
   * bot.utils.command.CommandClientBuilder#setScheduleExecutor(java.util.concurrent.ScheduledExecutorService)
  ...
  private void react(String reaction) {
    ...
    try {
      event
          .getMessage()
          .addReaction(Emoji.fromUnicode(reaction.replaceAll("<a?:(.+):(\\d+)>", "$1:$2")))
          .queue();
  ...
  /**
   * Gets a {@link SelfUser SelfUser} representing the bot. <br>
  ...
  /**
   * Gets a {@link Member Member} representing the bot, or null if the event does not take place on
   * a {@link Guild Guild}. <br>
  ...
  /**
   * Tests whether or not the {@link User User} who triggered this event is an owner of the bot.
   *
  ...
  /**
   * Gets the {@link MessageChannel MessageChannel} that the CommandEvent was triggered on.
   *
  ...
  /**
   * Gets the {@link ChannelType ChannelType} of the {@link MessageChannel MessageChannel} that the
  ...
  /**
   * Gets the {@link Guild Guild} that this CommandEvent was triggered on.
   *
  ...
  /**
   * Gets the instance of {@link JDA JDA} that this CommandEvent was caught by.
   *
  ...
  /**
   * Gets the {@link Message Message} responsible for triggering this CommandEvent.
   *
  ...
  /**
   * Gets the {@link PrivateChannel PrivateChannel} that this CommandEvent may have taken place on,
   * or {@code null} if it didn't happen on a PrivateChannel.
   *
  ...
  /**
   * Gets the {@link TextChannel TextChannel} that this CommandEvent may have taken place on, or
   * {@code null} if it didn't happen on a TextChannel.
   *
  ...
  /**
   * Compares a provided {@link ChannelType ChannelType} with the one this CommandEvent occurred on,
