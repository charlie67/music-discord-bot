 ...
 * MessageContextMenu#execute(MessageContextMenuEvent)} body:
 *
 ...
 * }</code></pre>
 *
 * <p>Execution is with the provision of a MessageContextInteractionEvent-CommandClient wrapper
 * called a {@link MessageContextMenu} and is performed in two steps:
 *
 * <ul>
 *   <li>{@link MessageContextMenu#run(MessageContextMenuEvent) run} - The menu runs through a
 *       series of conditionals, automatically terminating the command instance if one is not met,
 *       and possibly providing an error response.
 *   <li>{@link MessageContextMenu#execute(MessageContextMenuEvent) execute} - The menu, now being
...
public class MessageContextMenuEvent extends MessageContextInteractionEvent {
  ...
  public MessageContextMenuEvent(
      @NotNull JDA api,
      long responseNumber,
      @NotNull MessageContextInteraction interaction,
   ...
   *
   * <p>The {@link ReplyCallbackAction} returned by sending the response as a {@link Message}
   * automatically does {@link ReplyCallbackAction#queue() RestAction#queue()}.
   *
   ...
   * automatically does {@link ReplyCallbackAction#queue() RestAction#queue()}.
   *
   * <p>This method uses {@link
   * GenericCommandInteractionEvent#replyFiles(net.dv8tion.jda.api.utils.FileUpload...)} to send the
   * File. For more information on what a bot may send using this, you may find the info in that
   * method.
   *
  ...
  public void respond(File file, String filename, String description, boolean spoiler) {
    ...
    if (description != null && !description.isEmpty()) fileUpload.setDescription(description);
  ...
  public boolean isOwner() {
    if (getUser().getId().equals(this.getClient().getOwnerId())) return true;
    if (this.getClient().getCoOwnerIds() == null) return false;
    for (String id : this.getClient().getCoOwnerIds())
      if (id.equals(getUser().getId())) return true;
    return false;
  }
}
