...
/**
 * Middleware for child context menu types. Anything that extends this class will inherit the
 * following options.
 *
 * @author Olivia (Chew)
 */
public abstract class ContextMenu extends Interaction {
  /**
   * The name of the command. This appears in the context menu. Can be 1-32 characters long. Spaces
   * are allowed.
   *
  ...
  /**
   * Localization of menu names. Allows discord to change the language of the name of menu in the
   * client.
   */
  ...
  public Command.Type getType() {
    if (this instanceof MessageContextMenu) return Command.Type.MESSAGE;
    else if (this instanceof UserContextMenu) return Command.Type.USER;
    else return Command.Type.UNKNOWN;
  }
  ...
  /**
   * Gets an error message for this Context Menu under the provided {@link
   * GenericCommandInteractionEvent}.
   *
   * @param event The event to generate the error message for.
   * @param remaining The remaining number of seconds a context menu is on cooldown for.
   * @param client the client
   * @return A String error message for this menu if {@code remaining > 0}, else {@code null}.
   */
  public String getCooldownError(
      GenericCommandInteractionEvent event, int remaining, CommandClient client) {
  ...
  /**
   * Builds CommandData for the ContextMenu upsert. This code is executed when we need to upsert the
   * menu.
   *
